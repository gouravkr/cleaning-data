...
#Clean the feature names and make them unique for use as column labels
...
subject_test <- read.table("UCI HAR Dataset/test/subject_test.txt", header = F) # Row wise subject name

# Use the features table to label the columns of the test dataset
names(x_test) <- c(as.vector(features$V2))

# Using the activity labels table, add a second column to the activity table 
# to represent the activity name instead of the activity number
...
# The following part will require the user to load the dplyr package.
#Use install.packages("dplyr") to install the package if it is not already installed.
library(dplyr)
...
# In the first step, select only those columns which contain mean or std in the column name.
# Also select the subject and activity name columns
...
# In case the dplyr package is not available or cannot be loaded for any reason,
# use the following code to summarize the dataset     

subjects <- 1:30 #create the subject vector
activities <- as.vector(act_labels$V2) #create a vector with activity names

#Create a logical vector of column numbers containing the terms mean and std
col_in <- grepl("mean|std", names(alldata), ignore.case=T) 
col_in[1:2] <- TRUE #set first two values to TRUE for first two columns
...
# This data frame's first column contains the subject names. 
# Since each subject will appear 6 times, 1:30 has been repeated six times.
# To keep the same subject names together, the vector has been sorted in ascending order.
...
# The following for() loop works at three levels
...
# Once all the activities for a particular subject are exhausted, 
     # the loop exits the third level and at the second level, moves to the next subject
# Once all subjects are exhausted at the second level, the loop exits 
     # the second level and at the first level, the next column is selected
# At this point, row number is reset back to 1 so that the summary of next column starts from first row
